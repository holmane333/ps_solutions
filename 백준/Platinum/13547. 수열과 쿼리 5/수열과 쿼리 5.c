...
int main() {
	...
	for (int i = q[1].a; i <= q[1].b; ++i) if (++visit[ary[i]] == 1) ++cnt;
	res[q[1].idx] = cnt;
	for (int i = 2; i <= m; ++i) {
		long long ns = q[i].a, ne = q[i].b;
		for (int j = s; j < ns; ++j) if (--visit[ary[j]] == 0) --cnt;
		for (int j = ns; j < s; ++j) if (++visit[ary[j]] == 1) ++cnt;
		for (int j = e + 1; j <= ne; ++j) if (++visit[ary[j]] == 1) ++cnt;
		for (int j = ne + 1; j <= e; ++j) if (--visit[ary[j]] == 0) --cnt;
		res[q[i].idx] = cnt;
		s = ns;
		e = ne;
	}
