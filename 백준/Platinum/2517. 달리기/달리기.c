...
int main(void) {
	...
	for (int i = 1; i <= n; i++) {
		ary2[ary[i].idx].data = ary[i].data;
		ary2[ary[i].idx].idx = i;
	}
	//for (int i = 1; i <= n; i++) printf("%lld %d\n", ary2[i].data, ary2[i].idx);
	//puts("");
	for (int i = 1; i <= n; i++){
		printf("%lld\n", sum(1, 1, n, ary2[i].idx + 1, n) + 1LL);
		update(1, 1, n, ary2[i].idx, 1LL);
	}
	return 0;
}
