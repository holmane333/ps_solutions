...
void dfs(int now, int prev) {
	...
	for (int i = 0; i < len[now]; ++i) {
		if (adj[now][i] == prev) continue; // visit check
		dfs(adj[now][i], now);
	}
	E[now] = cnt;
}
...
int main(void) {
	...
	for (int i = 1; i <= n; i++) {
		...
		if (ary[i] == -1) continue;
		++len[ary[i]];
		adj[ary[i]] = realloc(adj[ary[i]], sizeof(int) * len[ary[i]]);
		adj[ary[i]][len[ary[i]] - 1] = i;
	}
