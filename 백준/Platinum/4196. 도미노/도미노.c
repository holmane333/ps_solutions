...
int main() {
	...
	while (tc--) {
		...
		ft = 0;
		scanf("%d %d", &n, &e);
		for (int i = 0; i <= 100000; ++i) dir[i] = (int*)malloc(sizeof(int)), idir[i] = (int*)malloc(sizeof(int)), scc[i].a = (int*)malloc(sizeof(int));
		for (int i = 0; i < e; ++i) {
			int a, b;
			scanf("%d %d", &a, &b);
		...
		for (int i = 0; i < idx; ++i) {
			for (int j = 0; j < scc[i].len; ++j) {
				sccn[scc[i].a[j]] = i + 1;
				//printf("%d ", scc[i].a[j], sccn[scc[i].a[j]]);
			}
			//puts("");
		}
		int c = 0, res = 0;
		for (int i = 0; i < idx; ++i) {
			...
			for (int j = 0; j < scc[i].len; ++j) {
				for (int k = 0; k < ilen[scc[i].a[j]]; ++k) {
					if (sccn[scc[i].a[j]] != sccn[idir[scc[i].a[j]][k]]) c = 1;
				}
			}
			if (!c) {
				++res;
		...
		for (int i = 0; i <= 100000; ++i) free(dir[i]), free(idir[i]), free(scc[i].a), scc[i].len = 0;
	}
	return 0;
}