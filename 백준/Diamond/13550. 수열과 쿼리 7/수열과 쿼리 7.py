...
for i in range(len(l)): l[i] %= k
d = dict()
t = list(set(l))
t.sort()
for idx, val in enumerate(t): d[val] = idx
for idx, val in enumerate(l): l[idx] = d[val]
sqrtn = int(sqrt(n))
m = int(stdin.readline())
q = [list(map(int, stdin.readline().split())) + [i] for i in range(m)]
for i in range(len(q)): q[i][0] -= 1
q.sort(key=lambda x:(x[0] // sqrtn, x[1]))

dq = [deque() for i in range(200010)]
sqrtq = 400
cnt_sqrt = [0 for i in range(200010//sqrtq + 10)]
cnt = [0 for i in range(200010)]
